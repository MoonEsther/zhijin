...
- **组织（Organization，用户补充需求 2026-08-21）**：
  - **模型**：租户 → 组织树 → 用户（`sys_organization` 表：tenant_id + parent_id 层级；`sys_user` 加 `org_id`；`sys_org_role` 组织-角色关联）
  - **授权继承**：用户 perms = 用户角色 perms ∪ **组织角色 perms**（RbacApplicationService 查询合并）
  - **V1 范围**：后端数据模型 + 组织 CRUD 接口 + 组织级角色授权；**前端组织管理页 V1 不做**（用户未选）
...
## Task 4: 用量/审计页 + RBAC 后端（方案 C + 组织）+ 联调收尾
...
- **RBAC 后端（新增，方案 C）**：
  ...
  - Modify: `zhijin-server/zhijin-auth/.../seeder/AdminSeeder.kt`（R2：AdminSeeder 在 zhijin-auth 模块；创建 admin 角色 + 全权限 + 分配）
- **组织（Organization，用户补充需求）**：
  - Create: `zhijin-server/zhijin-auth/.../resources/db/migration/V6__org_schema.sql`（sys_organization + sys_user.org_id + sys_org_role）
  - Create: `zhijin-server/zhijin-auth/.../domain/organization/`（Organization + OrganizationRepository）
  - Create: `zhijin-server/zhijin-auth/.../application/OrgApplicationService.kt`
  - Create: `zhijin-server/zhijin-auth/.../interfaces/OrgController.kt`
  - Modify: `zhijin-server/zhijin-auth/.../application/RbacApplicationService.kt`（perms = 用户角色 ∪ 组织角色）
...
`domain/organization/Organization.kt`（富血：id/tenantId/parentId/orgName/sort）+ `OrganizationRepository`（findByTenant/tree/CRUD）。
`application/OrgApplicationService.kt`：组织 CRUD + 树查询 + 分配角色。
`interfaces/OrgController.kt`（`/api/orgs`，JWT 保护，`@PreAuthorize("hasAuthority('role:manage')")`）：组织树/CRUD/分配角色。

- [ ] **Step 7: 权限继承合并（用户角色 ∪ 组织角色）**

`RbacApplicationService.getPerms(tenantId, userId)`：
```kotlin
// 用户直接角色 → perms
val userRoles = roleRepository.findRolesByUserId(tenantId, userId)
// 用户所属组织 → 组织角色 → perms（组织继承）
val orgRoles = roleRepository.findRolesByOrgId(tenantId, user.orgId ?: 0)
// 合并去重
val allRoles = (userRoles + orgRoles).distinctBy { it.id }
val perms = allRoles.flatMap { it.perms }.distinct()
```
`SecurityConfig.tokenCustomizer` 写 perms 时调 `getPerms`（注入 RbacApplicationService 或 Repository）。

- [ ] **Step 8: AdminSeeder 增强（方案 C + 组织）**

创建默认组织（根组织）+ `admin` 角色（全权限）+ 分配 `sys_user_role` + `sys_user.org_id` 指向根组织。
...
git commit -m "feat(web): 用量/审计页 + RBAC后端(权限点/@PreAuthorize/角色/组织)"
```
