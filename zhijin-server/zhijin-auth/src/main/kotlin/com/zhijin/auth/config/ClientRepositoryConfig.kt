 ...
 * 1) zhijin-console —— 前端控制台，authorization_code + PKCE（requireProofKey=true）授权码流程，
 *    登录后 redirect 回 http://localhost:5173/callback，不要求用户手动授权（requireAuthorizationConsent=false）。
 * 2) zhijin-server —— 服务间 M2M，client_credentials 客户端凭证模式。
 *
 * 说明：B2 重构由 JDBC 持久化改为内存注册（配合表单登录 + 授权码流程）。
 *       client secret 使用 passwordEncoder(BCrypt) 编码存储——授权服务器会用同一个
...
class ClientRepositoryConfig {
    ...
    fun registeredClientRepository(passwordEncoder: PasswordEncoder): RegisteredClientRepository {
        val console = RegisteredClient.withId("zhijin-console")
            ...
            .clientSecret(passwordEncoder.encode("console-secret"))
