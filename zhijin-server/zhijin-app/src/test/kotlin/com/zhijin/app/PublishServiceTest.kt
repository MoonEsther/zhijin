...
class PublishServiceTest {
    ...
    fun `首次发布版本号为1`() {
        ...
        `when`(appMapper.selectById(1L)).thenReturn(AppRecord(id = 1L, tenantId = 1L, appKey = "app_x", name = "x", status = 0))
        `when`(versionMapper.selectCount(any())).thenReturn(0L)
        // 模拟 MyBatis-Plus insert 后回填自增主键 id（IdType.AUTO 的真实行为）
        `when`(versionMapper.insert(any(AppVersionRecord::class.java))).thenAnswer(object : Answer<Int> {
            override fun answer(invocation: InvocationOnMock): Int {
                invocation.getArgument<AppVersionRecord>(0).id = 99L
                return 1
            }
        })
    ...
    fun `第二次发布版本号为2`() {
        ...
        `when`(appMapper.selectById(1L)).thenReturn(AppRecord(id = 1L, tenantId = 1L, appKey = "app_x", name = "x", status = 1))
