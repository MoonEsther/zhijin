...
**Tech Stack:** Spring Boot 4.0.0 · Spring Cloud 2025.1 · MyBatis-Plus `mybatis-plus-spring-boot4-starter` 3.5.15 · PostgreSQL 16 · Flyway 10.x · Testcontainers（测试）
...
- **测试**：Testcontainers 起真实 PostgreSQL（需本机 Docker）；若 Docker 不可用，降级 H2 PostgreSQL 兼容模式跑单测。
...
- [ ] **Step 2: 写集成测试（Testcontainers 起真实 PG）**

`TenantInterceptorIntegrationTest.kt`（验证 SQL 层租户隔离：查询自动带 tenant_id，插入自动填充）：
```kotlin
package com.zhijin.framework.tenant
...
配套实体与 Mapper（本测试所需，后续 B2/B3 复用）：
...
Run（需本机 Docker 起容器；若无 Docker 报告 BLOCKED 并在 README 注明）：
```bash
cd zhijin-server && mvn -pl zhijin-framework test -Dtest=TenantInterceptorIntegrationTest
```
Expected: `1 passed`，验证租户 SQL 级隔离生效。

- [ ] **Step 4: Commit**

```bash
git add zhijin-server/
git commit -m "feat(schema): Flyway 基础表 + 租户隔离集成测试"
```
