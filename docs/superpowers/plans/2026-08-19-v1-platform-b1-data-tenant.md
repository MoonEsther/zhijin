...
以下为子代理实现时发现并修正的计划缺陷/环境差异，均已构建验证：

| # | 修正 | 原因 |
|---|---|---|
| 1 | `zhijin-common/pom.xml` 需补 `spring-web` / `spring-context` / `slf4j-api` | `GlobalExceptionHandler` 用到 Spring Web 注解与 slf4j，原 pom 仅 JUnit（版本由 Boot BOM 管理） |
| 2 | `zhijin-framework/pom.xml` 需补 `spring-boot-starter-web` | 框架模块缺 servlet API / `HttpFilter`（Boot 4 需显式 starter） |
| 3 | `LogConfig.kt` 用 `setFilter(...)` 而非 `filter = ...` | Kotlin 2.2 + Boot 4 下 `AbstractFilterRegistrationBean.filter` 被视为只读 |
| 4 | T3 测试断言移入 `FilterChain` 回调内；透传用例用 `TRACE_ID_HEADER` 而非 MDC key | 过滤器 `finally` 清理 MDC，doFilter 返回后断言必 null；请求头名与 MDC key 不同 |
| 5 | `zhijin-framework/pom.xml` 补 `mybatis-plus-jsqlparser-4.9:3.5.15` | MyBatis-Plus 3.5.9+ JSqlParser 拆分独立构件（租户/分页拦截器所在），starter 不传递引入 |
| 6 | `zhijin-framework/pom.xml` 补 `spring-boot-starter-flyway` | Spring Boot 4 移除 `flyway-core` 自带的自动配置，需显式 starter |
| 7 | `MybatisPlusConfig` 加 `@MapperScan("com.zhijin.framework.mapper")` | 自动 Mapper 扫描仅覆盖 `@SpringBootApplication` 所在包（com.zhijin.app） |
| 8 | `MyMetaObjectHandler` 用 `Long::class.javaObjectType` 而非 `Long::class.java` | Kotlin 里 `Long::class.java` 是 primitive `long.class`，类型匹配失败导致 tenant_id 填充为 NULL（潜伏 bug，真插库才暴露） |
| 9 | 集成测试用 H2（PG 兼容模式），无需 Docker；schema 用 `GENERATED BY DEFAULT AS IDENTITY` 跨库可移植 | 本机无 Docker，Testcontainers 不可用 |
